...
class Server:
    ...
    def _handle_client(self, client_socket: socket.socket):
        logger.warning(
            f"RUNNING NEW THREAD CLIENT: {client_socket.getsockname()} - THREAD -- {th.current_thread().name}")
        while True:
            try:
            ...
            try:
                ...
                try:
                    if data != b'':  # NAME PLAYER
                        ...
                        current = \
                        list(set(range(self._max_players)) - set([position for position, _ in self._data.items()]))[0]
